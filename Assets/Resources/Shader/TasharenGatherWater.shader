Shader "Tasharen/TasharenGatherWater"
{
	Properties
	{
	    ...
	    _Radius ("Radius", Float) = 0.5
		 ...
		 _Direction("Water Direction", Float) = 0.0
		 _minDegrees("MinDegrees",Float)=30.0
	}
	...
	#endif
		// Phong shading model
		half reflectiveFactor = max(0, dot(-viewDir, reflect(lightDir, nNormal)));
   ...
   // Calculate the object-space normal (Z-up)
      ...
      float d = distance(_Center.xz, IN.pos.xz);
   float nowDegrees =_Direction;
   float trueDegrees =_Direction;
   float t = 100;
   if(d>_Radius){
   float3 _TempNormal= (_Center-IN.pos.xyz)/d;
   trueDegrees =degrees(atan2(_TempNormal.z,_TempNormal.x));
   t = fmod(trueDegrees,_minDegrees);
   nowDegrees =trueDegrees - t;
   }
   			float time = _Time.y * _Speed;
			half2 tiling = _Tiling * IN.worldPos.zx;
			float radian = radians(nowDegrees+135);
			float radian2 = radians(trueDegrees+135);
			float2 temp1 =  float2( sin(radian) + cos(radian),cos(radian) - sin(radian))* time ;
			float2 temp2 =  float2( sin(radian2) + cos(radian2),cos(radian2) - sin(radian2))* time ;
			float2 uv =lerp(temp1,temp2,0.5f) + tiling ;//float2( sin(radian) + cos(radian),cos(radian) - sin(radian)) * time + tiling;
			half4 uvNoise = tex2D(_WaterTex, uv) * 0.2;
			//half4 nmap = tex2D(_WaterTex,uvNoise.xy*_Tiling);
