...
{
        static UIRectEditor() {
        UIWidget.setHandlesFun = SetShowHandlesWithMoveTool;
        UIWidget.getHandlesFun = GetShowHandlesWithMoveTool;
        UIWidget.drawGizmosFun = OnDrawGizmos;
        UIWidget.showHandlesFun = showHandles;
        NGUITools.setDirtyFun = UnityEditor.EditorUtility.SetDirty;
        NGUITools.saveFun = UnityEditor.AssetDatabase.SaveAssets;
    }

    private static int mHandles = -1;
    static public void SetShowHandlesWithMoveTool(bool value) {
        int val = value ? 1 : 0;
        if (mHandles != val) {
            mHandles = val;
    ...
    static public bool GetShowHandlesWithMoveTool() {
        if (mHandles == -1) {
    ...
    public static void OnDrawGizmos(UIWidget widget) {
        if (widget.isVisible && NGUITools.GetActive(widget)) {
            if (UnityEditor.Selection.activeGameObject == widget.gameObject && showHandles()) return;

            Color outline = new Color(1f, 1f, 1f, 0.2f);

            float adjustment = (widget.root != null) ? 0.05f : 0.001f;
            Vector2 offset = widget.pivotOffset;
            Vector3 center = new Vector3(widget.width * (0.5f - offset.x), widget.height * (0.5f - offset.y), -widget.depth * adjustment);
            Vector3 size = new Vector3(widget.width, widget.height, 1f);

            // Draw the gizmo
            Gizmos.matrix = widget.cachedTransform.localToWorldMatrix;
            Gizmos.color = (UnityEditor.Selection.activeGameObject == widget.cachedTransform) ? Color.white : outline;
